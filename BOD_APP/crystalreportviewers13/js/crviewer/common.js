...
/**
 * @param hrefaray  Array of url of css files
 * @param callback function that gets executed once all css files are loaded
 */
bobj.includeCSSLinksAndExecuteCallback = function (hrefaray, callback) {
    if(hrefaray == null || hrefaray.length < 1) {
        callback.apply();
        return;
    }
    ...
    cb.hrefCount = hrefaray.length;
    cb.callback = callback;
    
    for(var i = 0, len = hrefaray.length; i < len; i++) {
        bobj.includeCSSLinkAndExecuteCallback(hrefaray[i], cb);
    }
};
