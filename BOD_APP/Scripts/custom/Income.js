...
function GetAllMonths() {
    ...
    $.ajax({
        ...
        success: function (data) {
            var outputHtml = "<div class='dynamic-column-item'>Accounting Code</div><div class='dynamic-column-item'>Type</div><div class='dynamic-column-item'>Contributor</div><div class='dynamic-column-item'>Description</div>";
            var outputHtml2 = '<select class="form-control input-sm  dynamic-column-item-ddl accounting" id="ddlAccounting"></select><select class="form-control input-sm drop_down dynamic-column-item-ddl"  onchange="FillContributor(this);"  id="ddlType"></select><select class="form-control input-sm  drop_down2 dynamic-column-item-ddl" id="ddlContributor"></select><input type="text" id="txtDesc" class="form-control input-sm  thtxt frm rate desc ">';
            if (data.length > 0) {
...
$(document).off('click', "#add-row").on('click', '#add-row', function (e) {
    var TypeId = $(this).closest('.dynamicColumnsInputs').find('#ddlType').val();
    var ContributorId = $(this).closest('.dynamicColumnsInputs').find('#ddlContributor').val();
    ...
    if (BudgetRow.length > 0) {
        ...
        dataHtml += '<select class="form-control input-sm  drop_down dynamic-column-item-ddl" id="ddlAccounting_' + rowCount + '"></select><select class="form-control input-sm  drop_down dynamic-column-item-ddl" onchange="FillContributor(this)" id="ddlType_' + rowCount + '"></select><select class="form-control input-sm  drop_down2 dynamic-column-item-ddl" id="ddlContributor_' + rowCount + '"></select><input type="text" id="txtDesc_' + rowCount + '" class="form-control input-sm  thtxt frm rate desc ">';
...
$(document).off('click', '.btnSave').on('click', '.btnSave', function (e) {
    ...
    if (count > 0) {
        ...
        $.ajax({
            type: "POST",
            url: "/ScriptJson/InsertUpdateIncomeBudget",
            data: _data,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (data) {
...
function retrive(id) {
    ...
    var _data = JSON.stringify({
        global: {
            ...
            StoreProcedure: 'IncomeBudget_USP',
        }
    });

    $.ajax({
        type: "POST",
        url: '/ScriptJson/GetIncomeBudgets',
        data: _data,
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        async: false,
            ...
            if (data.BudgetReturnData.length > 0) {
                ...
                for (var i = 0; i < data.BudgetReturnData.length; i++) {
                    if (tempIndex.length == 0) {
                        tempIndex.push(data.BudgetReturnData[i].IB_Group);
                    }
                    else {
                        if (tempIndex.indexOf(data.BudgetReturnData[i].IB_Group) == -1) {
                            tempIndex.push(data.BudgetReturnData[i].IB_Group);
                        }
                    }
                }
                for (var j = 0; j < tempIndex.length; j++) {
                    ...
                    var Accounting = 0;
                    for (var i = 0; i < data.BudgetReturnData.length; i++) {
                        if (tempIndex[j] == data.BudgetReturnData[i].IB_Group) {
                            TypeId = data.BudgetReturnData[i].IB_typeId;
                            ContributorId = data.BudgetReturnData[i].IB_ContributorId;
                            description = data.BudgetReturnData[i].IB_Description;
                            Accounting = data.BudgetReturnData[i].IB_Accounting;
                            MonthList.push({
                                "Id": data.BudgetReturnData[i].IB_MonthId,
                                "Year": data.BudgetReturnData[i].IB_Year,
                                "Value": data.BudgetReturnData[i].IB_Amount
                            });
                        }
